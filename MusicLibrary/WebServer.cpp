...
MyHTTPSrv::MyHTTPSrv(wchar_t* pDomain, wchar_t* pPort)
{
    if (pDomain)
        wcscpy_s(aDomain, pDomain);
    else
        wcscpy_s(aDomain, L"localhost");
    if (pPort)
        wcscpy_s(aPort, pPort);
    else
        wcscpy_s(aPort, L"8080");

    wcscpy_s(aURL, L"http://");
    wcscat_s(aURL, aDomain);
    wcscat_s(aURL, L":");
    wcscat_s(aURL, aPort);
    wcscat_s(aURL, L"/");
...
void MyHTTPSrv::SetGetFunc(string(*pGetFuncW)(PCWSTR))
{
    if (pGetFuncW)
        aGetFuncW = pGetFuncW;
} // SetGetFunc

void MyHTTPSrv::SetGetFunc(string(*pGetFuncS)(string))
{
    if (pGetFuncS)
        aGetFuncS = pGetFuncS;
} // SetGetFunc
...
--*******************************************************************/
DWORD MyHTTPSrv::DoReceiveRequests(
    IN HANDLE hReqQueue
)
{
    ULONG              result;
    ...
    //
    RequestBufferLength = sizeof(HTTP_REQUEST) + 20480;
    ...
    for (;;)
    {
        ...
        if (NO_ERROR == result)
        {
            ...
            case HttpVerbGET:
                ...
                lWCaller = pRequest->CookedUrl.pFullUrl;
                lCaller = string(lWCaller.begin(), lWCaller.end());

                lRes = "Hey! You hit the server \r\n";
                if (aGetFuncS) {
                    //if (lCaller != "http://localhost:8080/favicon.ico") {
                    lRes = aGetFuncS(lCaller);
...
DWORD MyHTTPSrv::SendHttpResponse(
    IN HANDLE        hReqQueue,
...
)
{
    ...
    if (pMimeType == "text/html") {
    ...
    else if (pMimeType == "image/x-icon") {
        //
        // Add a known header.
        //
        ...
        if (pEntityString)
        {
            ifstream lFile(pEntityString);
            if (lFile)
            {
                lFile.seekg(0, ios::end);
                lFLen = lFile.tellg();
                lFile.seekg(0, ios::beg);
                lFBuffer = new char[lFLen];
                lFile.read(lFBuffer, lFLen);
            }

            dataChunk.DataChunkType = HttpDataChunkFromMemory;
            dataChunk.FromMemory.pBuffer = (PVOID)lFBuffer;
            dataChunk.FromMemory.BufferLength = (ULONG)(lFLen);
            response.EntityChunkCount = 1;
            response.pEntityChunks = &dataChunk;
        }
    }
...
DWORD MyHTTPSrv::SendHttpPostResponse(
    IN HANDLE        hReqQueue,
    IN PHTTP_REQUEST pRequest
)
{
